      ...
      <div class="w-9/12 truncate">
        <small
          ...
          @click="
            router.push({
              name: 'artistDetail',
              query: { id: song.ar[0].id },
            })
          "
          >{{ song.ar[0].name }}</small
        >
      </div>
    </div>
    <div
...
const { id } = toRefs(state.player);

const play = function (id: number) {
  dispatch("player/play", id);
};
const downMusics = async function (id: number) {
  const res = await downMusic(id);
